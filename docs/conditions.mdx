...
Accepts a single entity type or a list; matches if any listed type matches.
...
### block_type
Checks the type of block involved in the event (e.g., in block break/place events).

Accepts a single block material or a list; matches if any listed material matches.
...
### javascript
Evaluate a JavaScript expression or script using GraalVM. The following bindings are available:
- plugin: PanopticPlugin instance
- event: the current Bukkit Event
- player: the Player (if resolvable from the event)

A script is considered true if it returns:
- a boolean true
- a non-zero number
- a non-empty string
...
- type: script
  value: |
    // Example: only allow in survival world and when sneaking
    event.getPlayer() != null &&
    event.getPlayer().getWorld().getName() === 'survival' &&
